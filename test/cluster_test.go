...
func TestAddListener(t *testing.T) {
	...
	wg.Add(1)
	registrationID := client.ClusterService().AddListener(&membershipListener{wg: wg})
	member, _ := remoteController.StartMember(cluster.ID)
	timeout := WaitTimeout(wg, Timeout)
	assert.Equalf(t, nil, false, timeout, "Cluster initialMembershipListener failed")
	client.ClusterService().RemoveListener(registrationID)
	wg.Add(1)
...
func TestAddListeners(t *testing.T) {
	...
	client, _ := hazelcast.NewClient()
	wg.Add(2)
	registrationID1 := client.ClusterService().AddListener(&membershipListener{wg: wg})
	registrationID2 := client.ClusterService().AddListener(&membershipListener{wg: wg})
	remoteController.StartMember(cluster.ID)
	timeout := WaitTimeout(wg, Timeout)
	assert.Equalf(t, nil, false, timeout, "Cluster initialMembershipListener failed")
	client.ClusterService().RemoveListener(registrationID1)
	client.ClusterService().RemoveListener(registrationID2)
	client.Shutdown()
	remoteController.ShutdownCluster(cluster.ID)
}
...
func TestGetMembers(t *testing.T) {
	...
	client, _ := hazelcast.NewClient()
	members := client.ClusterService().GetMembers()
...
func TestGetMember(t *testing.T) {
	...
	member := client.ClusterService().GetMember(address)
...
func TestRestartMember(t *testing.T) {
	...
	wg.Add(1)
	registrationID := client.LifecycleService().AddListener(&lifecycleListener)
	remoteController.ShutdownMember(cluster.ID, member1.UUID)
	timeout := WaitTimeout(wg, Timeout)
	...
	client.LifecycleService().RemoveListener(registrationID)
	client.Shutdown()
	remoteController.ShutdownCluster(cluster.ID)
}
...
func TestReconnectToNewNodeViaLastMemberList(t *testing.T) {
	...
	memberList := client.ClusterService().GetMembers()
