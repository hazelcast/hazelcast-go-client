...
func TestMutualAuthentication(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			muStatus: REQUIRED,
			ca:       "testdata/OpenSSL/rootCA.crt",
		...
		{
			name:     "testMA_OptionalWithNoCertFile",
			muStatus: OPTIONAL,
			ca:       "testdata/OpenSSL/rootCA.crt",
			cert:     "",
			key:      "",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.wantErr {
				require.Error(t, sslTestUtil(t, tc.muStatus, tc.ca, tc.cert, tc.key))
			} else {
				require.NoError(t, sslTestUtil(t, tc.muStatus, tc.ca, tc.cert, tc.key))
			}
		})
	}
}

func sslTestUtil(t *testing.T, mu MutualAuthenticationStatus, ca, cert, key string) error {
	ctx := context.Background()
	port := it.NextPort()
	clsConfig := xmlSSLMutualAuthenticationConfig(t.Name(), port, mu)
           ...
           http://www.hazelcast.com/schema/config/hazelcast-config-4.0.xsd">
			...
			<network>
				...
				<ssl enabled="true">
					...
					<properties>
						<property name="keyStore">testdata/OpenSSL/ca.keystore</property>
						<property name="keyStorePassword">123456</property>
						<property name="trustStore">testdata/OpenSSL/ca.truststore</property>
