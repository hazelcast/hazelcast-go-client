 ...
 * TODO DOC
 */
//@Generated("3b106df02154a9ec6818f78a0a38a928")
const (
    //hex: 0x000200
...
func ClientAuthenticationCustomEncodeRequest(clusterName string, credentials serialization.Data, /* @Nullable */ uuid UUID, clientType string, serializationVersion byte, clientHazelcastVersion string, clientName string, labels Collection<String>, partitionCount int, /* @Nullable */ clusterId UUID) *bufutil.ClientMessagex {
...
func ClientAuthenticationCustomDecodeRequest(clientMessage *bufutil.ClientMessagex) *ClientAuthenticationCustomRequestParameters {
    iterator := clientMessage.FrameIterator()
    request := new(ClientAuthenticationCustomRequestParameters)
    initialFrame := iterator.Next()
    request.uuid = bufutil.DecodeUUID(initialFrame.Content, ClientAuthenticationCustomRequestUuidFieldOffset)
    request.serializationVersion = bufutil.DecodeByte(initialFrame.Content, ClientAuthenticationCustomRequestSerializationVersionFieldOffset)
    request.partitionCount = bufutil.DecodeInt(initialFrame.Content, ClientAuthenticationCustomRequestPartitionCountFieldOffset)
    request.clusterId = bufutil.DecodeUUID(initialFrame.Content, ClientAuthenticationCustomRequestClusterIdFieldOffset)
...
func ClientAuthenticationCustomEncodeResponse(status byte , /* @Nullable */ address Address , /* @Nullable */ uuid UUID , serializationVersion byte , serverHazelcastVersion string , partitionCount int , clusterId UUID ) *bufutil.ClientMessagex {
    clientMessage := bufutil.CreateForEncode()
	initialFrame := bufutil.Frame{make([]byte, ClientAuthenticationCustomResponseInitialFrameSize), bufutil.UnfragmentedMessage}
    bufutil.EncodeInt(initialFrame.Content, bufutil.TypeFieldOffset, ClientAuthenticationCustomResponseMessageType)
    bufutil.EncodeByte(initialFrame.Content, ClientAuthenticationCustomResponseStatusFieldOffset, status)
    bufutil.EncodeUUID(initialFrame.Content, ClientAuthenticationCustomResponseUuidFieldOffset, uuid)
    bufutil.EncodeByte(initialFrame.Content, ClientAuthenticationCustomResponseSerializationVersionFieldOffset, serializationVersion)
    bufutil.EncodeInt(initialFrame.Content, ClientAuthenticationCustomResponsePartitionCountFieldOffset, partitionCount)
    bufutil.EncodeUUID(initialFrame.Content, ClientAuthenticationCustomResponseClusterIdFieldOffset, clusterId)
    clientMessage.Add(initialFrame)
