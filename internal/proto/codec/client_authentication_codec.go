...
func EncodeClientAuthenticationRequest(clusterName string, username string, password string, uuid types.UUID, clientType string, serializationVersion byte, clientHazelcastVersion string, clientName string, labels []string) *proto.ClientMessage {
	...
	EncodeUUID(initialFrame.Content, ClientAuthenticationCodecRequestUuidOffset, uuid)
	EncodeByte(initialFrame.Content, ClientAuthenticationCodecRequestSerializationVersionOffset, serializationVersion)
...
func DecodeClientAuthenticationResponse(clientMessage *proto.ClientMessage) (status byte, address *cluster.Address, memberUuid types.UUID, serializationVersion byte, serverHazelcastVersion string, partitionCount int32, clusterId types.UUID, failoverSupported bool) {
	...
	status = DecodeByte(initialFrame.Content, ClientAuthenticationResponseStatusOffset)
	memberUuid = DecodeUUID(initialFrame.Content, ClientAuthenticationResponseMemberUuidOffset)
	serializationVersion = DecodeByte(initialFrame.Content, ClientAuthenticationResponseSerializationVersionOffset)
	partitionCount = DecodeInt(initialFrame.Content, ClientAuthenticationResponsePartitionCountOffset)
	clusterId = DecodeUUID(initialFrame.Content, ClientAuthenticationResponseClusterIdOffset)
	failoverSupported = DecodeBoolean(initialFrame.Content, ClientAuthenticationResponseFailoverSupportedOffset)
