...
type Decoder func(frameIterator *proto.ForwardFrameIterator) pubserialization.Data
...
func EncodeNullableForData(message *proto.ClientMessage, data pubserialization.Data) {
	if data == nil {
		message.AddFrame(proto.NullFrame.Copy())
	} else {
...
func DecodeNullableForData(frameIterator *proto.ForwardFrameIterator) pubserialization.Data {
...
func DecodeNullableForAddress(frameIterator *proto.ForwardFrameIterator) *pubcluster.Address {
	if NextFrameIsNullFrame(frameIterator) {
		return nil
	}
...
func DecodeNullableForLongArray(frameIterator *proto.ForwardFrameIterator) []int64 {
	if NextFrameIsNullFrame(frameIterator) {
		return nil
	}
...
func DecodeNullableForSimpleEntryView(frameIterator *proto.ForwardFrameIterator) *types.SimpleEntryView {
	if NextFrameIsNullFrame(frameIterator) {
		return nil
	}
...
func EncodeData(message *proto.ClientMessage, value interface{}) {
	message.AddFrame(proto.NewFrame(value.(pubserialization.Data)))
}

func EncodeNullableData(message *proto.ClientMessage, data pubserialization.Data) {
	if data == nil {
		message.AddFrame(proto.NullFrame.Copy())
	} else {
		message.AddFrame(proto.NewFrame(data))
	}
}

func DecodeData(frameIterator *proto.ForwardFrameIterator) pubserialization.Data {
	return frameIterator.Next().Content
}

func DecodeNullableData(frameIterator *proto.ForwardFrameIterator) pubserialization.Data {
	if NextFrameIsNullFrame(frameIterator) {
		return nil
	}
	return DecodeData(frameIterator)
...
func EncodeEntryListForDataAndListData(message *proto.ClientMessage, entries []proto.Pair) {
	...
	for _, value := range entries {
		EncodeData(message, value.Key)
		EncodeListData(message, value.Value.([]pubserialization.Data))
	}
	message.AddFrame(proto.EndFrame.Copy())
}
...
func DecodeEntryListUUIDListInteger(frameIterator *proto.ForwardFrameIterator) []proto.Pair {
	...
	for i := 0; i < keySize; i++ {
		result[i] = proto.Pair{Key: keys, Value: values}
	}
	return result
}
...
func EncodeListMultiFrame(message *proto.ClientMessage, values []pubserialization.Data, encoder Encoder) {
	message.AddFrame(proto.NewBeginFrame())
...
func EncodeListMultiFrameForData(message *proto.ClientMessage, values []pubserialization.Data) {
	message.AddFrame(proto.NewBeginFrame())
	for i := 0; i < len(values); i++ {
...
func EncodeListMultiFrameContainsNullable(message *proto.ClientMessage, values []pubserialization.Data, encoder Encoder) {
	message.AddFrame(proto.NewBeginFrame())
	for i := 0; i < len(values); i++ {
		if values[i] == nil {
...
func EncodeListMultiFrameNullable(message *proto.ClientMessage, values []pubserialization.Data, encoder Encoder) {
	if len(values) == 0 {
		message.AddFrame(proto.NullFrame)
	} else {
...
func DecodeListMultiFrameForData(frameIterator *proto.ForwardFrameIterator) []pubserialization.Data {
	result := make([]pubserialization.Data, 0)
	frameIterator.Next()
	for !NextFrameIsDataStructureEndFrame(frameIterator) {
...
func DecodeListMultiFrameForDataContainsNullable(frameIterator *proto.ForwardFrameIterator) []pubserialization.Data {
	result := make([]pubserialization.Data, 0)
	frameIterator.Next()
	for !NextFrameIsDataStructureEndFrame(frameIterator) {
		if NextFrameIsNullFrame(frameIterator) {
...
func EncodeListData(message *proto.ClientMessage, entries []pubserialization.Data) {
	EncodeListMultiFrameForData(message, entries)
}

func DecodeListData(frameIterator *proto.ForwardFrameIterator) []pubserialization.Data {
