 ...
 */
//@Generated("b856b7139350b338644844ca52e798ac")
const (
    //hex: 0x050B00
...
func ListAddListenerDecodeRequest(clientMessage *bufutil.ClientMessagex) *ListAddListenerRequestParameters {
    iterator := clientMessage.FrameIterator()
    request := new(ListAddListenerRequestParameters)
    initialFrame := iterator.Next()
    request.includeValue = bufutil.DecodeBoolean(initialFrame.Content, ListAddListenerRequestIncludeValueFieldOffset)
    request.localOnly = bufutil.DecodeBoolean(initialFrame.Content, ListAddListenerRequestLocalOnlyFieldOffset)
...
func ListAddListenerEncodeResponse(response UUID ) *bufutil.ClientMessagex {
    clientMessage := bufutil.CreateForEncode()
	initialFrame := bufutil.Frame{make([]byte, ListAddListenerResponseInitialFrameSize), bufutil.UnfragmentedMessage}
    bufutil.EncodeInt(initialFrame.Content, bufutil.TypeFieldOffset, ListAddListenerResponseMessageType)
    bufutil.EncodeUUID(initialFrame.Content, ListAddListenerResponseResponseFieldOffset, response)
    clientMessage.Add(initialFrame)
...
func ListAddListenerDecodeResponse(clientMessage *bufutil.ClientMessagex) *ListAddListenerResponseParameters {
    ...
    func encodeListAddListenerEvent(/* @Nullable */ item serialization.Data, uuid UUID, eventType int) *bufutil.ClientMessagex {
        clientMessage := bufutil.createForEncode()
    	initialFrame := bufutil.Frame{make([]byte, ListAddListenerEventItemInitialFrameSize), bufutil.UNFRAGMENTED_MESSAGE}
        initialFrame.flags |= ClientMessage.IS_EventFLAG
        bufutil.EncodeInt(initialFrame.Content, bufutil.TypeFieldOffset, ListAddListenerEventItemMessageType)
        encodeUUID(initialFrame.Content, ListAddListenerEventItemUuidFieldOffset, uuid)
        encodeInt(initialFrame.Content, ListAddListenerEventItemEventTypeFieldOffset, eventType)
        clientMessage.Add(initialFrame)
    ...
    type ListAddListenerAbstractEventItemFunc func(/* @Nullable */ item serialization.Data, uuid UUID, eventType int)
