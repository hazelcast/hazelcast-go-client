 ...
 */
//@Generated("91007c8eb3c27589c82bd7ec0378806b")
const (
    //hex: 0x011D00
...
func MapGetEntryViewDecodeRequest(clientMessage *bufutil.ClientMessagex) *MapGetEntryViewRequestParameters {
    iterator := clientMessage.FrameIterator()
    request := new(MapGetEntryViewRequestParameters)
    initialFrame := iterator.Next()
    request.threadId = bufutil.DecodeLong(initialFrame.Content, MapGetEntryViewRequestThreadIdFieldOffset)
...
type MapGetEntryViewResponseParameters struct {
    ...
    * TODO DOC
    */
/* @Nullable */ response [serialization.Data][serialization.Data]SimpleEntryView
    /**
    * TODO DOC
    */
maxIdle int64
...
func MapGetEntryViewEncodeResponse(/* @Nullable */ response [serialization.Data][serialization.Data]SimpleEntryView , maxIdle int64 ) *bufutil.ClientMessagex {
    clientMessage := bufutil.CreateForEncode()
	initialFrame := bufutil.Frame{make([]byte, MapGetEntryViewResponseInitialFrameSize), bufutil.UnfragmentedMessage}
    bufutil.EncodeInt(initialFrame.Content, bufutil.TypeFieldOffset, MapGetEntryViewResponseMessageType)
    bufutil.EncodeLong(initialFrame.Content, MapGetEntryViewResponseMaxIdleFieldOffset, maxIdle)
    clientMessage.Add(initialFrame)

    CodecUtil.encodeNullable(clientMessage, response, SimpleEntryViewCodecEncode)
    return clientMessage
}
...
func MapGetEntryViewDecodeResponse(clientMessage *bufutil.ClientMessagex) *MapGetEntryViewResponseParameters {
    ...
    response.response = CodecUtil.decodeNullable(iterator, SimpleEntryViewCodecDecode) 
    return response
}
