...
```
go get github.com/hazelcast/hazelcast-go-client/v3
```

or this with the `-v` flag for verbose output

```
go get -v github.com/hazelcast/hazelcast-go-client/v3
```

This will pull down the Hazelcast client for Go from Github, from [here](https://github.com/hazelcast/hazelcast-go-client/v3.git), and
...
├── go/src/github.com/hazelcast/hazelcast-go-client/v3
```
...
From the command line, in the directory with the `one.go` program (`$GOPATH/src/github.com/hazelcast/hazelcast-go-client/v3/samples/hello-world/one`)
run this command
