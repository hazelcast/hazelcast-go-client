...
TEST_FLAGS ?= -v -count 1 -timeout 25m -tags=hazelcastinternal,hazelcastinternaltest
...
test-all:
	env MEMBER_COUNT=$(MEMBER_COUNT) HZ_VERSION=4.2.2 go test $(TEST_FLAGS) $(PACKAGES) ./...
