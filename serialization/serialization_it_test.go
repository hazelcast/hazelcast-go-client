...
type colorCompactSerializer struct{}

func (colorCompactSerializer) Type() reflect.Type {
	return reflect.TypeOf(color{})
}

func (c colorCompactSerializer) TypeName() string {
	return c.Type().Name()
}

func (colorCompactSerializer) Read(reader *serialization.CompactReader) interface{} {
	return color{
		Red:   reader.ReadInt32("red"),
		Green: reader.ReadInt32("green"),
		Blue:  reader.ReadInt32("blue"),
	}
}

func (colorCompactSerializer) Write(writer *serialization.CompactWriter, value interface{}) {
	c, ok := value.(color)
	if !ok {
		panic("not a color")
	}
	writer.WriteInt32("red", c.Red)
	writer.WriteInt32("green", c.Green)
	writer.WriteInt32("blue", c.Blue)
}

func TestCompactSerialize(t *testing.T) {
	it.MapTesterWithConfig(t, func(config *hz.Config) {
		config.Serialization.Compact.SetSerializers(&colorCompactSerializer{})
	}, func(t *testing.T, m *hz.Map) {
		target := color{Red: 100, Green: 200, Blue: 300}
		it.Must(m.Set(context.Background(), "mycolor", target))
		if value, err := m.Get(context.Background(), "mycolor"); err != nil {
			t.Fatal(err)
		} else if !reflect.DeepEqual(target, value) {
			t.Fatalf("target %#v: %#v", target, value)
		}
