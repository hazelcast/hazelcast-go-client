...
func TestTopic_Publish(t *testing.T) {
	it.TopicTester(t, func(t *testing.T, tp *hz.Topic) {
		...
		subscriptionID, err := tp.AddListener(nil, func(event *hz.MessagePublished) {
			handlerValue.Store("value1")
		})
		if err != nil {
			t.Fatal(err)
		}
		if err = tp.Publish(nil, "HEY!"); err != nil {
			t.Fatal(err)
		}
		time.Sleep(1 * time.Second)
		assert.Equal(t, "value1", handlerValue.Load())
		if err := tp.RemoveListener(nil, subscriptionID); err != nil {
			t.Fatal(err)
		}
		handlerValue.Store("base-value")
		if err = tp.Publish(nil, "HEY!"); err != nil {
...
func TestTopic_PublishAll(t *testing.T) {
	it.TopicTester(t, func(t *testing.T, tp *hz.Topic) {
		...
		if err = tp.PublishAll(nil, "v1", "v2", "v3"); err != nil {
			t.Fatal(err)
		}
		time.Sleep(1 * time.Second)
		...
		if err = tp.PublishAll(nil, "v4", "v5", "v6"); err != nil {
			t.Fatal(err)
		}
		time.Sleep(1 * time.Second)
		assert.Equal(t, int32(3), atomic.LoadInt32(&handlerValue))
