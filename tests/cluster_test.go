...
func TestRestartMember(t *testing.T) {
	...
	AssertEqualf(t, nil, lifecycleListener.collector[0], internal.LifecycleStateDisconnected, "clusterService reconnect has failed")
	wg.Add(1)
	remoteController.StartMember(cluster.ID)
	timeout = WaitTimeout(wg, Timeout)
	AssertEqualf(t, nil, false, timeout, "clusterService reconnect has failed")
	AssertEqualf(t, nil, lifecycleListener.collector[1], internal.LifecycleStateConnected, "clusterService reconnect has failed")
...
func TestConnectToClusterWithoutPort(t *testing.T) {
	...
	client, _ := hazelcast.NewHazelcastClientWithConfig(config)
	members := client.GetCluster().GetMemberList()
	AssertEqualf(t, nil, members[0].Address().Host(), "127.0.0.1", "connectToClusterWithoutPort returned a wrong member address")
